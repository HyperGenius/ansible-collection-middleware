...
Zabbix Server構築完了後に、**Zabbix API**を介して論理的な設定（自動登録ルール、ホストグループ作成など）を行うAnsible Role。
...
`ansible_connection: httpapi` と `community.zabbix.zabbix` ネットワークOSプラグインを使用してZabbix APIと通信を行います。
...
```yaml
# Zabbix API接続情報
zabbix_config_server_hostname: "localhost"
zabbix_config_server_port: "80"
zabbix_config_url_path: ""  # 例: "/zabbix" (ルート直下でない場合)
zabbix_config_api_user: "Admin"
zabbix_config_httpapi_use_ssl: false
zabbix_config_httpapi_validate_certs: false
...
- `zabbix_config_server_hostname`: Zabbix Serverのホスト名またはIPアドレス
- `zabbix_config_server_port`: Zabbix Serverのポート番号 (HTTP/HTTPS)
- `zabbix_config_url_path`: Zabbix Web UIのパス (例: `/zabbix`。ルート直下の場合は空文字)
- `zabbix_config_api_user`: API接続用のユーザー名（デフォルトはAdmin）
- `zabbix_config_httpapi_use_ssl`: SSL(HTTPS)を使用するかどうか
- `zabbix_config_httpapi_validate_certs`: SSL証明書の検証を行うかどうか
- `zabbix_config_api_password`: API接続用のパスワード
- `zabbix_config_autoreg_name`: 自動登録アクションの名前
- `zabbix_config_autoreg_metadata`: エージェント側の `HostMetadata` とマッチさせる文字列
- `zabbix_config_autoreg_group`: 自動登録時に追加（作成）するホストグループ
...
- hosts: localhost
  ...
  roles:
    - role: middleware.middleware.zabbix_config
      vars:
        zabbix_config_server_hostname: "zabbix-server.example.com"
        zabbix_config_url_path: "/zabbix"
...
- hosts: localhost
  ...
  tasks:
    ...
    - ansible.builtin.include_role:
      ...
      vars:
        zabbix_config_server_hostname: "{{ hostvars[groups['monitoring_servers'][0]]['ansible_host'] }}"
        zabbix_config_url_path: "/zabbix"
        zabbix_config_api_password: "{{ vault_zabbix_admin_password }}"
```
...
エージェントを起動すると、自動的にZabbix Serverに登録され、HostMetadataが一致するため `Linux servers`グループに追加され、`Linux by Zabbix agent`テンプレートが適用されます。
