...
def test_zabbix_service_running(host, zabbix_vars):
    ...
    service_name = zabbix_vars["zabbix_server_core_service_name"]
...
def test_zabbix_port_listening(host, zabbix_vars):
    """Zabbix Serverが指定されたポートでリッスンしていることを確認する"""
    listen_port = zabbix_vars["zabbix_server_core_listen_port"]

    # 指定されたポートでリッスンしていることを確認
...
def test_zabbix_log_no_database_errors(host, zabbix_vars):
    ...
    log_file = zabbix_vars["zabbix_server_core_log_file"]
