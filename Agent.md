...
- **JSON/YAML:** 設定ファイルを生成する際は、可能な限り構文チェックを行うこと。
  - **Linux:** `template` モジュールの `validate` オプションを使用する。（例: `python3 -m json.tool %s`）
  - **Windows:** `win_template` 実行後に `win_shell` で `ConvertFrom-Json` 等を用いて検証タスクを実行する。
